...
public class NetModificationDialog extends NetworkListDialog {
	...
	protected void initControls(final String title, final boolean showWarning) {
		...
		final JLabel warningLbl = new JLabel(SM_NETMODIFICATION, SwingConstants.LEADING);
		...
		btnOK = Utils.createButton(new AbstractAction("OK") {
			@Override
		...
		btnCancel = Utils.createButton(new AbstractAction("Cancel") {
			@Override
			public void actionPerformed(ActionEvent e) {
				setVisible(false);
				dispose();
